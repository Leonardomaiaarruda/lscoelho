...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const modoImpressao = document.getElementById('modoImpressao');
    const selectColab = document.getElementById('selectColaborador');

    if (modoImpressao && selectColab) {
       modoImpressao.addEventListener('change', () => {
            // 1. Reseta o seletor de colaborador
            selectColab.value = ""; 
            selectColab.options[0].text = "-- Colaborador --";
            
            // 2. Limpa os campos de cabeçalho (Incluindo agora o ANO)
            const camposCabecalho = [
                'c-nome', 'c-funcao', 'c-cpf', 'c-adm', 'c-matri', 'c-ctps', 'c-ano',
                'c2-nome', 'c2-funcao', 'c2-cpf', 'c2-adm', 'c2-matri', 'c2-ctps', 'c2-ano'
            ];
            camposCabecalho.forEach(id => {
                const el = document.getElementById(id);
                if (el) {
                    el.value = ""; // Remove o "2026" ou qualquer outro valor
                    el.style.backgroundColor = ""; 
                    el.style.color = "";
                }
            });

            // 3. Limpa TODA a tabela de EPIs (20 linhas)
            for (let i = 0; i < 21; i++) {
                ['dev-', 'data-', 'desc-', 'qtd-', 'fab-', 'ca-', 'val-'].forEach(prefixo => {
                    const campo = document.getElementById(prefixo + i);
                    if (campo) campo.value = "";
                });
            }

            // 4. Reseta Assinatura e Foto
            const sig = document.getElementById('nome-assinatura');
            if (sig) sig.innerText = "NOME DO COLABORADOR";
            ...
            // 5. Atualiza a visibilidade (Modo Fantasma)
...
async function preencher() {
    ...
    if(typeof adm === 'number') {
            // Converte o número serial do Excel para objeto Date
            let dataObj = new Date(Math.round((adm - 25569) * 864e5));
            
            // CORREÇÃO: Adiciona o fuso horário local para evitar que o dia retroceda
            dataObj.setMinutes(dataObj.getMinutes() + dataObj.getTimezoneOffset());
            
            // Formata para o padrão brasileiro
            adm = dataObj.toLocaleDateString('pt-BR');
    }
